...
export class CodeParser {
  ...
  private findCallsInBlock(node: {
    getDescendantsOfKind: (kind: SyntaxKind) => any[];
  }): FunctionCall[] {
  ...
  private getProps(file: SourceFile): ComponentProp[] {
    ...
    const component =
      file.getFunctions().find(f => f.isExported()) ||
      file
        .getVariableDeclarations()
        .find(
          v =>
            v.isExported() &&
            (v.getInitializer()?.isKind(SyntaxKind.ArrowFunction) ||
              v.getInitializer()?.isKind(SyntaxKind.FunctionExpression)),
        );
        ...
        return jsDocs
          .map(
            doc =>
              ...
              doc
                .getTags()
                .map(t => `@${t.getTagName()} ${t.getComment() || ''}`)
                .join('\n'),
          )
          .join('\n');
      }
