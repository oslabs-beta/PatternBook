...
export function generateMermaid(data: GraphData, options: { minImports?: number; focusTarget?: string } = {}): string {
    ...
    mermaidCode += '        L_Focus["Focus"]:::component ~~~ L_Imp["High Imports"]:::highImports ~~~ L_API["High API"]:::highApi ~~~ L_Logic["High Logic"]:::highLogic ~~~ L_DB[("Database")]:::database ~~~ L_Minor["Minor"]:::leaf\n';
    mermaidCode += '    end\n';
    ...
    else if (layerUI.length > 0 && layerData.length > 0) mermaidCode += '    UI_Layer ~~~ Data_Layer\n';
...
export function convertParsedFilesToGraph(files: ParsedFile[]): GraphData {
    ...
    files.forEach(file => {
        ...
        node.metrics.functionCalls = file.functions.reduce((acc, func) => acc + func.calls.length, 0);
