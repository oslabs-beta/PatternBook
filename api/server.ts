...
//   console.log('DEBUG: typeof manifestCache.components =', typeof manifestCache.components);
//   console.log('DEBUG: Array.isArray(manifestCache.components) =', Array.isArray(manifestCache.components));
//   console.log('DEBUG: manifestCache.components.length =', manifestCache.components?.length);
// };
// DEBUG CODE END
...
app.get('/api/graph', (req: Request, res: Response) => {
  if (!dependencyGraphCache) {
    return res.status(404).json({
      ...
      message: 'Run `patternbook analyze` to create a dependency graph',
    });
  }
...
app.get('/api/graph/impact/:componentName', (req: Request, res: Response) => {
  ...
  const directDependents = dependencyGraphCache.edges
    ?.filter((e: any) => e.to === node.filePath)
    .map((e: any) => {
...
app.get('/api/search', (req: Request, res: Response) => {
  ...
  const results = manifestCache.components.filter((c: any) => {
    ...
    const matchTags = c.tags?.some((tag: string) =>
